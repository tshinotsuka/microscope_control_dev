...
- **命名 / NAS 構造**（→ `nas_structure.md`・必要なら README / `data_structure.md` の index）: dev/test の project_id を
  **`2026_microscope_control_dev` に確定**（`<YYYY>_<theme>` 準拠。リポジトリ名 `microscope_control_dev` とは別）。
  **dev/test データも本実験同様 NAS に上げる**方針 → 解析側ツリー（`<user>/projects/<project_id>/...`）に dev project_id が並ぶ。
  取得側 SOP（`scanimage_logfilestem_sop.md`）は反映済み。解析側で project_id 例・該当 index を同期。
  既存 `microscope_control_dev` データは raw リネーム禁止の原則どおり**そのまま**、新規分から新 id。
- 改訂履歴に 1 行。
...
**目標: 方式A を実機で end-to-end 実装・検証し、契約どおりの behavior + imaging テスト 1 本を出し、
...
3. **Aux Trigger セットアップ**：injector TTL を Aux Trigger に接続、imaging と同時記録。注入を模擬して
   TIFF に on/off timestamp が載るか確認。〔ゲート: TIFF/metadata に注入 timestamp〕
4. **同期検証**：imaging + Data Recorder + Aux Trigger を 1 本同時録り。start を揃え、
   frame ↔ behavior ↔ injection の対応を確認。〔ゲート: 対応が想定どおり〕
5. **契約適合**：logFileStem で sub/ses/cond/run を入れ、同 stem・`raw/` に出るか、metadata が契約どおりかを確認。
   〔ゲート: gate 条件 2〕
...
**セッション後**：確定手順から `sops/behavior_datarecorder_sop.md` を起こし、`status.md` を更新
（go なら behavior 入力経路を ☑、§3 は fallback 注記だけ残す）。
