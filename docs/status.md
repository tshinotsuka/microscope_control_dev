...
- → **本番前ハードウェアゲートは①②とも完了**。残りは Tier-0 の契約凍結・SI metadata 実機検証・behavior/injector を契約に載せる（§2.1 / roadmap §4）。**injection GUI v1 完了（2026-06-18・`syncControlPanel`・§2.2/§3.5）**＝in-vivo 前の打ち間違い防止。残り近場＝R1 sidecar VALID／cycle_period の hSI 自動取得（現状ハードコード 8ms）。
...
- **次段（GUI）＝v1 完了（2026-06-18）**: `N`/pulse/enable を入れて arm/disarm する**薄い GUI**（config 前面・ロジックは UF 側・§2.2／`syncControlPanel`）。`N=目標cycle−1` を表示で吸収。cycle period は現状ハードコード → **hSI 自動取得が TODO**。
